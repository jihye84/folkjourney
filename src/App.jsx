...
function App() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-950 font-sans select-none">
      ...
      <div className="absolute top-0 left-0 w-full p-6 pb-16 bg-gradient-to-b from-slate-950/90 via-slate-950/40 to-transparent pointer-events-none z-10 flex justify-between items-start">
        <div className="pointer-events-auto">
          <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400 drop-shadow-lg">
            Folk Journey
          </h1>
          <p className="text-[11px] text-slate-400 font-medium mt-0.5 tracking-wide drop-shadow">
          ...
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            ...
            {/* World Tour Button with Solid Opaque Background & Neon Border Glow */}
            <button
              onClick={() => setIsWorldTour(!isWorldTour)}
              className={`flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl border backdrop-blur-xl transition-all shadow-xl
                ${isWorldTour
            ...
            <button
              ...
              className="p-2 bg-slate-900/90 hover:bg-white/10 text-slate-400 hover:text-white rounded-xl border border-white/15 backdrop-blur-xl transition-colors shadow-xl"
              title="단축키 안내"
            >
      ...
      {isWorldTour && (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 pointer-events-auto">
          <div className="flex items-center gap-3 px-4 py-2 bg-slate-900/95 backdrop-blur-xl border border-cyan-500/50 rounded-full shadow-[0_0_20px_rgba(6,182,212,0.3)]">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-cyan-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-cyan-500"></span>
              </span>
              <span className="text-xs font-bold text-cyan-300">✈️ 세계 여행 탐험 중</span>
            </div>
