        ...
        >
          ...
          <div className="flex justify-between items-center px-4 py-2.5 md:px-5 md:py-3 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="h-7 md:h-9 shrink-0 flex items-center">
                <img
              ...
              <div className="min-w-0">
                <h2 className="text-sm md:text-lg font-black text-white leading-tight break-keep">{region.title}</h2>
                <p className="text-[10px] md:text-xs text-emerald-400 font-bold leading-tight mt-0.5 break-keep">
                  {isMobile ? region.description : region.region}
                </p>
              </div>
            </div>
          ...
          <div className="flex px-3.5 pt-1 md:hidden shrink-0 bg-slate-950/40 border-b border-white/10 gap-2">
            <button
              onClick={() => setActiveTab('music')}
              className={`flex items-center gap-1.5 px-3 py-1 text-[11px] font-bold border-b-2 transition-all ${
                activeTab === 'music'
          ...
          {/* ── Desktop View (Rich & Spacious 460px Panel) vs Mobile View (Compact Zero-Scroll) ── */}
          {!isMobile ? (
            /* ── DESKTOP PC VIEW: Rich, Spacious, Complete Layout ── */
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4">
              {/* Prominent Summary Description Card */}
              ...
              {/* Controls Bar: Key + BPM + Style + Play */}
              <div className="flex items-center gap-2 p-2.5 bg-slate-800/50 border border-white/10 rounded-xl">
                ...
                <button onClick={handlePlayAll}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-xl font-bold text-xs md:text-sm transition-all shrink-0 shadow
                    ${isPlaying
                  ...
                  {region.chords.map((chord, idx) => {
                    ...
                    return (
                      <motion.button key={idx}
                        ...
                        className={`flex-1 min-w-[76px] relative rounded-xl flex flex-col items-center justify-between p-1.5 transition-all duration-300 border min-h-[92px] overflow-hidden group
                        ...
                        <div className="px-1.5 py-px rounded text-[8px] font-bold border border-slate-700 mb-0.5 bg-slate-900 text-slate-400">
                          {chord.role || chord.numeral}
                        </div>
                        <span className={`text-base font-black leading-tight ${isActive || isSingle ? 'text-emerald-400' : 'text-slate-200'}`}>
                          {chord.numeral}
                        </span>
                        <span className="text-[10px] font-bold text-amber-500 leading-tight">
                          {getChordName(selectedKey, chord.numeral)}
                        </span>
                        <div className="w-full bg-white rounded-md p-0.5 my-0.5 flex items-center justify-center overflow-hidden shrink-0 shadow-inner h-[54px]">
                        ...
                        {!isPlaying && !isActive && !isSingle && (
                          <div className="absolute inset-0 group-hover:bg-emerald-500/10 rounded-xl flex items-center justify-center transition-colors">
                            <Volume2 className="w-4 h-4 text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                          </div>
                        )}
                        <AnimatePresence>
                          {(isActive || isSingle) && (
              ...
              {region.story && (
                <div className="space-y-2 pt-2 border-t border-white/10">
                  <h4 className="text-xs md:text-sm font-bold text-slate-300 flex items-center gap-1.5">
            ...
            /* ── MOBILE SMARTPHONE VIEW: Compact Bottom Sheet ── */
